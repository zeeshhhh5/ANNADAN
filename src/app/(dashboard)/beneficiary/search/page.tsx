...
export default async function BeneficiarySearchPage() {
  const session = await auth();

  if (!session?.user || session.user.role !== "BENEFICIARY") {
    redirect("/dashboard");
  }

  // Fetch available food from API
  const response = await fetch(`${process.env.NEXTAUTH_URL || "http://localhost:3000"}/api/beneficiary/search`, {
    cache: "no-store",
  });
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-white">Find Food</h1>
        <p className="text-gray-400 mt-1">Search for available food donations near you</p>
      </div>
        ...
        <div className="flex-1">
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-gray-900 border border-gray-800 rounded-lg pl-10 pr-4 py-2 text-white placeholder:text-gray-500 focus:border-green-500 focus:outline-none"
            />
          </div>
        </div>
        <select className="bg-gray-900 border border-gray-800 rounded-lg px-4 py-2 text-white focus:border-green-500 focus:outline-none">
      ...
      {listings.length === 0 ? (
        <Card className="bg-gray-900 border-gray-800">
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <h3 className="text-xl font-semibold text-white mb-2">No food available</h3>
